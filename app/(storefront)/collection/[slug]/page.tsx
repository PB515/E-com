...
import { getStoreCollection, getCollectionProducts } from "@/lib/collections";
    ...
    <>
      <section className="mx-auto max-w-[1400px] px-5 pb-4 pt-12 sm:px-8 lg:pt-16">
        <p className="text-sm text-ink-muted">
          <a href="/shop" className="hover:text-ink">Shop</a> / Collection
        </p>
        <h1 className="mt-3 font-heading text-4xl text-ink lg:text-5xl">{collection.title}</h1>
      ...
      <section className="mx-auto max-w-[1400px] px-5 py-10 sm:px-8">
        {products.length > 0 ? (
          ...
          <div className="rounded-3xl border border-border bg-surface px-8 py-16 text-center">
            <p className="font-heading text-2xl text-ink">Nothing in this collection yet</p>
            <p className="mx-auto mt-2 max-w-sm text-sm text-ink-muted">Check back soon, or browse the full collection.</p>
            <a href="/shop" className="mt-6 inline-block text-sm text-primary underline-offset-4 hover:underline">Shop all</a>
          </div>
        )}
      </section>
