...
## Current status
**Bucket 2 in progress (OMS/CRM). Slices 1 & 2 LIVE in prod; Slice 3 built + verified, committed local (`main`, NOT pushed).** Slice 2 = variants + stock-movement ledger + manual/draft/marketplace orders (migration 0005 applied to shared DB), deployed. Slice 3 = storefront variant picker + variant-aware cart/checkout + `/admin/inventory` stock-ledger view; verified end-to-end (real order rolled back), **no migration**, safe to push anytime. Held decisions: pieces mostly have variants; stock lives on variants, `products.stock` is a derived mirror; registered state currently **Gujarat** (confirm before launch — drives CGST/SGST vs IGST). NEXT: marketplace reports (2B), coupons/back-in-stock/abandoned-cart/GA4 (2D).
