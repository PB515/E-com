...
## Current status
**Phase 0 DONE (green).** Scaffold built: Next.js 16 + Tailwind v4 + doc-04 tokens + all 03b routes + lib/adapter stubs; **prod build passes**; committed (`main` @ `f852c4c`, `phase-0` branch). Skills in `.claude/skills/` (incl. experimental `taste-skill` — Bugadi is its verification build, a Phase-1 compare task). Image prompts ready in `docs/image-prompts.md` (34 files; product/hero/category/roots = real photography before launch). **Next: Phase 1 (Homepage)** — branch `phase-1` off main; wire real fonts via `frontend-design`. Parallel human track: Vercel blank deploy + Step-5 accounts/keys (Supabase Mumbai, Razorpay test, Shiprocket, Resend) in `.env.local`.
