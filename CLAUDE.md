...
## Current status
**Phase 1 DONE (green).** Homepage built with `design-taste-frontend`: near-black/silver/rose tokens, Cormorant+Outfit fonts, header/footer, 6 home sections; storefront under `app/(storefront)/`. Prod build passes, verified live (1280 + 360px), no console errors. New deps: `motion`, `@phosphor-icons/react`. **Next: Phase 2** — remaining storefront pages static (category, **PDP w/ Heritage Story block**, cart, our-roots, policy pages); branch `phase-2` off main. Reuse `ui/Button`, `ui/ImageSlot`, `site/Reveal`, `lib/catalog`. Storefront = taste-skill; admin/checkout do NOT use it. **Still open (your side):** Vercel 404 (set Framework Preset = Next.js → redeploy) + Step-5 accounts/keys for Phase 4. Image slots are placeholders → real photos from `docs/image-prompts.md` before launch.
